...
| A5 | 畫線工具（已交付） | K 線可建立水平支撐／壓力線與兩點式趨勢線，owner-scoped 保存並跨裝置同步；趨勢線支援端點重新定位與日曆時間動態投影，首次 tick 建立 relation 基準、真正換側才觸發 `trend_cross_above/below`。幾何更新同步告警並重建基準；水平線維持 `price_above/below`，`alert_id` 保證冪等。 | `CandlestickChart.tsx` price-line／line-series API、D1 evaluator registry、`ChartDrawing` | migrations 0080–0081；`/charts/drawings*`;`trend_alert_evaluations_total`;無額外圖表套件 | L |
