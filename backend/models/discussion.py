...
class DiscussionTurn(Base):
    ...
    # B4 (migration 0069): true for turns that exist because the owner
    # interjected — the owner's question turn itself AND the persona
    # answer turn generated in direct response to a mid-round
    # interjection / post-conclusion 追問. Ordinary round-loop persona
    # turns stay false; the UI keys its 使用者提問 / 插話回覆 badges
    # off this flag.
    injected_by_user: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default=false(),
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False,
