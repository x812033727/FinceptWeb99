        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-0">
          ...
          {detail?.conclusion && (
            <div className="bg-amber-950/20 border border-amber-800/50 rounded-lg p-4 mt-4">
              ...
              {detail.conclusion._parse_error ? (
                <p className="text-xs text-red-400">
                  {t("discussion.conclusion_parse_error")}
                </p>
              ) : (
                <>
                  {detail.conclusion.recommended_symbols.length > 0 && (
                    <div className="mb-2">
                      <span className="text-xs text-muted-foreground">
                        {t("discussion.recommended_symbols")}：
                      </span>
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {detail.conclusion.recommended_symbols.map((s) => (
                          <span
                            key={s}
                            className="px-2 py-0.5 rounded bg-amber-900/30 text-amber-200 text-xs font-mono"
                          >
                            {s}
                          </span>
                        ))}
                  ...
                  <p className="text-sm text-foreground mb-2 whitespace-pre-wrap">
                    {detail.conclusion.reasoning}
                  </p>
                  {detail.conclusion.risks.length > 0 && (
                    <div className="mb-2">
                      <span className="text-xs text-muted-foreground">
                        {t("discussion.risks")}：
                      </span>
                      <ul className="mt-1 list-disc list-inside text-xs text-foreground/80 space-y-0.5">
                        {detail.conclusion.risks.map((r, idx) => (
                          <li key={idx}>{r}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    <span className="font-mono ml-1">
                      {(detail.conclusion.consensus_score * 100).toFixed(0)}%
                    </span>
                    <span className="ml-3">
                      {t("discussion.horizon")}：
                      <span className="ml-1">
                        {t(`discussion.horizon_${detail.conclusion.time_horizon}`)}
                      </span>
                    </span>
                  </div>
                </>
