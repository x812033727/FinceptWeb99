...
# lights for the four StatementDog-style categories. The implementation
# moved to `tw_health_metrics` (PR-C split); re-exported here so existing
# call sites — `test_tw_health_service.py` (svc.get_health / svc._safe_div
# / svc._light), `api/tw_market/router.py` consumer, and the patch in
# `test_tw_market_api.py` — keep working unchanged. The new module
# lazy-imports back into `tw_market_service.get_revenue` for the YoY
# headline so module-load order stays acyclic.
from services.tw_health_metrics import (  # noqa: E402, F401
    _BALANCE_ALIASES,
    _CASHFLOW_ALIASES,
    _INCOME_ALIASES,
    _light,
...
# Screener recovery tier + diagnostic helpers moved to
# `tw_screener_recovery` (PR-D split). Re-exported so any caller / test
# that grabbed these off `tw_market_service` keeps working. The DB-
# touching primitives below (`_bellwether_ohlcv_closes`,
# `_detect_stock_day_all_session`, `_independent_source_is_fresh`,
# `get_latest_ohlcv_session`, `_latest_complete_session`) stay here —
# `test_tw_market_service_staleness.py` mocks them via
# `patch.object(svc, ...)` to fake the DB, and a cross-module move
# would invalidate those patches' reach into the recovery functions.
# The orchestrators in `tw_screener_recovery` lazy-import the kept
# primitives at call time so the patches still resolve.
from services.tw_screener_recovery import (  # noqa: E402, F401
    _STALENESS_BELLWETHER,
    _TTL_SCREENER_STALE,
    _record_attempt,
    _recover_screener_via_finmind,
    _recover_screener_via_yfinance,
    _seed_diagnostic,
    _set_final_data_source,
    _yf_ticker_for,
)
...
# `_yf_ticker_for` + the two recovery orchestrators
# (`_recover_screener_via_yfinance`, `_recover_screener_via_finmind`)
# + the diagnostic sink helpers (`_record_attempt`,
# `_set_final_data_source`, `_seed_diagnostic`) live in
# `tw_screener_recovery` since PR-D. The names are re-exported above
# so existing import call sites resolve.
