"""Public data + catalog endpoints for the FinMind clone.

Three routes mounted under `/api/finmind`:

  GET  /catalog              — auth-free marketing catalog
  GET  /datasets             — full operator-visible catalog
  GET  /data/{code}          — paginated row query, FinMind-mirror shape
  GET  /data/{code}/export   — bulk CSV / NDJSON streaming export

Quota + scope gates apply to /data + /export (the row-charging paths);
catalog endpoints are free. All shared helpers (date coercion, scope
authorization, usage logging, streaming serialisers) live alongside
the routes that use them — they're deliberately not lifted to a
common utility module because no other route group consumes them.
"""
...
from finmind.api.auth import AuthContext, require_api_key
