...
function _signedPctSafe(n: number | null | undefined): string {
  if (n === null || n === undefined) return "—";
  const sign = n >= 0 ? "+" : "";
  return `${sign}${(n * 100).toFixed(2)}%`;
}

function _pctClass(n: number | null | undefined): string {
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[10px] text-muted-foreground">
          ...
          <span className="font-mono ml-1 text-foreground/80">
            {row.day1_open !== null ? _toFixedSmart(row.day1_open) : "—"}
          </span>
        </span>
      </div>
      <div className="grid grid-cols-5 gap-1">
        {row.change_pcts.map((pct, i) => (
          ...
          >
            ...
            <div className={`font-mono font-semibold ${_pctClass(pct)}`}>
              {_signedPctSafe(pct)}
            </div>
            <div className="font-mono text-muted-foreground/70">
              {row.daily_closes[i] !== null
                ? _toFixedSmart(row.daily_closes[i] as number)
                : "—"}
            </div>
          </div>
        ))}
      ...
      {open && (
        <div className="mt-2 space-y-2">
          ...
          {!hasConclusion ? (
            ...
            data.rows.map((r) => <ScoreboardRow key={r.symbol} row={r} />)
          )}
        </div>
      )}
    </div>
